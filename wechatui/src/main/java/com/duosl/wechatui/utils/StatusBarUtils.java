...
public class StatusBarUtils {
    ...
    public static StatusBarUtils with(Activity activity) {
        //为什么不用单例？
        //因为每个activity的statusBar可以是不同的,getWindow()也是不同的
    ...
    public StatusBarUtils setDarkTheme(boolean isDarkTheme) {
...
//            decorView.removeView(decorView.getChildAt(decorView.getChildCount()-1));
            L.d(decorView.getChildCount()+"");
        }
        //取消 if (mDrawable != null) 判断，当mDrawable重新为null时，移除背景色
//        addStatusViewWithDrawble(mActivity, mDrawable);
        if (mColor != -1) {
            //设置了状态栏颜色
